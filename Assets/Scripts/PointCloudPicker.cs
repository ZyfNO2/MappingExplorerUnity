...
public class PointCloudPicker : MonoBehaviour
{
    ...
    public void EnsureColliderExists()
    {
        if (pointCloudRenderer != null)
        {
            ...
            {
                // 检查是否已有碰撞器
                Collider collider = pointCloudRenderer.gameObject.GetComponent<Collider>();
                if (collider == null)
                {
                    // 为点云对象添加BoxCollider
                    BoxCollider boxCollider = pointCloudRenderer.gameObject.AddComponent<BoxCollider>();
                    
                    // 计算点云的边界框
                    if (pointCloudRenderer.vertices != null && pointCloudRenderer.vertices.Count > 0)
                    {
                        Bounds bounds = new Bounds(pointCloudRenderer.vertices[0], Vector3.zero);
                        foreach (Vector3 vertex in pointCloudRenderer.vertices)
                        {
                            bounds.Encapsulate(vertex);
                        }
                        boxCollider.center = bounds.center;
                        boxCollider.size = bounds.size;
                        Debug.Log($"Added BoxCollider to point cloud object with bounds: {bounds.size}");
                    }
                    else
                    {
                        // 如果没有顶点数据，使用默认大小
                        boxCollider.size = new Vector3(1, 1, 1);
                        Debug.Log("Added BoxCollider to point cloud object with default size.");
                    }
                }
            }
            catch (System.Exception e)
            {
                Debug.LogWarning($"EnsureColliderExists error: {e.Message}");
            }
        }
    }
